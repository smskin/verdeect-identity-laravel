...
- Решений о доступе: роль и ограничения пакет **предоставляет**
  (`CurrentIdentity`, см. [Встраивание](embedding.md)), но что за ними стоит,
  решает продукт в собственном слое авторизации. Украшатель профиля — не место
  для такого решения: он дополняет отображаемые данные, а не права.

## Прикладные права — решение продукта

Ролей в установке две, и прикладных признаков («диспетчер», «бухгалтер»)
сервис не знает. Такие ограничения продукт обеспечивает у себя.

Если продукту довольно роли из токена, обе точки расширения остаются пустыми:
`CurrentIdentity` и посредники маршрутов закрывают случай без них. Если нужны
собственные права — политики Laravel заводятся заглушками, разрешающими все
действия, и наполняются по мере появления условий. Это **решение конкретного
продукта, а не свойство пакета**: другой продукт той же установки вправе решить
иначе, ничего в библиотеке не меняя.
