...
return [
    ...
    /*
     * Наименьший промежуток между двумя принудительными обменами токена
     * одной сессии входа, секунды.
     *
     * Это не срок кэша: запись живёт затем, чтобы честный отказ
     * по недостаточным правам не звал установку на каждом запросе. Посредники
     * прав перед отказом один раз обменивают токен и перепроверяют право
     * (справка 9, «Дополнительно»), и без дросселя сотрудник, упорно
     * открывающий чужой экран, обменивал бы токен без конца.
     *
     * Ноль отключает дроссель и означает обмен перед каждым отказом.
     */
    'rights_probe_ttl' => (int) env('IDENTITY_RIGHTS_PROBE_TTL', 30),
