 ...
 * (`NavItem::$iconUrl`), и это накладывает на `identity.cache.services_ttl`
 * верхнюю границу: срок кэша обязан быть заметно меньше срока подписи,
 * который задаёт установка. Проверить соотношение в одиночку продукт
 * не в состоянии — срок подписи знает только установка, — поэтому она держит
 * его заметно выше поставляемого с пакетом часа и предупреждает при сближении.
 * Подняв свой срок, продукт обязан убедиться, что запас сохранился: ссылка,
 * протухшая раньше кэша, даёт битые иконки у всех его пользователей
 * до следующего промаха.
 */
final class NavigationClient
{
     ...
     * Ключ записи кэша.
     *
     * **Отметка состава (`v2`) — не украшение, а защита от молчаливой
     * деградации при обновлении пакета.** Запись кэша хранит уже разобранный
     * состав, а не сырой ответ, и переименование `icon` → `icon_url`
     * состав изменило. Разборщик 0.2, прочитав запись, положенную 0.1,
     * не находит `iconUrl` и подставляет пустую ссылку — законное значение,
     * означающее «ссылки не пришло», — и рейл показывает заглушки вместо
     * иконок у всех пользователей продукта до истечения срока кэша.
     *
     ...
     * сами по своему сроку. **Меняется она при всяком изменении состава
     * записи** — то есть при всякой правке `NavItem::toArray()`
     * и `NavigationData::toArray()`.
     */
    private function key(string $sub): string
    {
        return 'services:v2:'.$sub;
    }
}
