        ...
        switch ($event->type) {
            ...
            case self::TYPE_RIGHTS_CHANGED:
                if ($event->sub !== null) {
                     ...
                     * критерий 87).
                     *
                     * **Права помечаются устаревшими.** Они читаются из токена
                     * доступа, а не из сессии, поэтому до обмена продукт видит
                     * прежнюю роль — и повышение не применялось бы до истечения
                     * токена. Обмен по отметке выполняет ближайший запрос
                     * самого пользователя (`RefreshIdentityToken`): обработчик
                     * очереди только помечает и установку не зовёт
                     * (справка 8.1, критерий 73).
                     */
                    $this->caches->forgetServices($event->sub);
                    $this->marks->markRights($event->sub, $event->occurredAt);

                    Log::debug('[IdentityEventDispatcher.dispatch] rights marked stale', [
                        'sub' => $event->sub,
                    ]);
                }
